...
Sul client, chiama [`hydrateRoot`](/reference/react-dom/client/hydrateRoot) per idratare l'HTML generato sul server.
...
* `renderToString` ha un supporto limitato a Suspense. Se un componente sospende, `renderToString` invia immediatamente il suo fallback come HTML.
...
Questo produrrà l'output HTML iniziale non interattivo dei tuoi componenti React. Sul client, dovrai chiamare [`hydrateRoot`](/reference/react-dom/client/hydrateRoot) per *idratare* quell'HTML generato sul server e renderlo interattivo.
...
Se un componente sospende (ad esempio, perché è definito con [`lazy`](/reference/react/lazy) o recupera dati), `renderToString` non attenderà che il suo contenuto venga risolto. Invece, `renderToString` troverà il confine [`<Suspense>`](/reference/react/Suspense) più vicino sopra di esso e renderizzerà la sua prop `fallback` nell'HTML. Il contenuto non apparirà finché non viene caricato il codice client.
