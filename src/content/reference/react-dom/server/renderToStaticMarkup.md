...
* `reactNode`: Un nodo React che vuoi renderizzare in HTML. Ad esempio, un nodo JSX come `<Page />`.
...
* L'output di `renderToStaticMarkup` non può essere idratato.
...
Questo metodo renderizza **HTML non interattivo che non può essere idratato.** È utile se vuoi usare React come semplice generatore di pagine statiche, o se renderizzi contenuto completamente statico come email.
